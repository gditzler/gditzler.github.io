...
1. **`sass/_variables.scss`** — Defines raw palette colors (Solarized Dark and Solarized Light) under `[data-theme="dark"]` and `[data-theme="light"]` selectors, then maps them to semantic CSS custom properties (`--color-bg`, `--color-link`, etc.). All other SASS partials reference only semantic variables, never raw palette colors.
